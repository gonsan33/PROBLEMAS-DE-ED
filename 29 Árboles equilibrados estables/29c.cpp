...
template <typename T>
tuple<bool, int, int> f(const BinTree<T>& t) {
    if (t.empty()) return { true, 0,0 };
    else if (t.left().empty() && t.right().empty()) return { true, 1, 0 }; 
    else {
        auto [esI, hI, hpI] = f(t.left()); 
        auto [esD, hD, hpD] = f(t.right());
        return { esI && esD && abs(hI - hpD) <= 1 && abs(hD - hpI) <= 1, 1 + max(hI, hD), 1 + min(max(hI, hpD), max(hD, hpI))};
    }
}
...
bool es_estable(const BinTree<T>& arbol) {
    return get<0>(f(arbol));
}

void resuelveCasos() {
    BinTree<int> t; 
    cin >> t; 
    es_estable(t) ? cout << "SI\n" : cout << "NO\n"; 
}
