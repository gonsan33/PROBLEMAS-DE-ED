...
template <typename T> 
BinTree<T> reconstruir(const vector<T>& preorden, const vector<T>& inorden, int iniPre, int finPre, int iniIn, int finIn) {
    if (finPre <= iniPre) return BinTree<T>(); 
    else {
        int x = preorden[iniPre]; //raíz 

        //calculo la posición de la x en el inorden: 
        int posx = 0; 
        while (posx < inorden.size() && inorden[posx] != x) ++posx; 

        //Obtengo el árbol izquierdo 
        int iniInIzq = iniIn; 
        int finInIzq = posx; 
        int iniPreIzq = iniPre + 1; 
        int finPreIzq = iniPreIzq + (finInIzq - iniInIzq); 
        BinTree<T> hijoIzquierdo = reconstruir(preorden, inorden, iniPreIzq, finPreIzq, iniInIzq, finInIzq); 

        //Obtengo el árbol derecho 
        int iniInDcho = posx + 1; 
        int finInDcho = finIn; 
        int finPreDcho = finPre; 
        int iniPreDcho = finPreDcho - (finInDcho - iniInDcho); 
        BinTree<T> hijoDerecho = reconstruir(preorden, inorden, iniPreDcho, finPreDcho, iniInDcho, finInDcho); 

        return BinTree<T>(hijoIzquierdo, x, hijoDerecho); 

    }
}

BinTree<int> reconstruir(vector<int> const& preorden, vector<int> const& inorden) {
    return reconstruir(preorden, inorden, 0, preorden.size(), 0, inorden.size()); 
}
...
bool resuelveCasos() {
    ...
    for (auto& x : preorden) cin >> x; 
    for (auto& x : inorden) cin >> x;
