...
template <typename T> 
bool binary_search_tree(const BinTree<T>& t) {
    if (t.empty()) return true; 
    else {
        auto prev = t.begin(); 
        auto act = ++t.begin(); 

        while (act != t.end() && *prev < *act) {
            
            
            //prev = act; ojo, que esto puede tener coste lineal en el tamaño de la pila 
            
            ++prev; 
            ++act; //también es mejor ++act que act++, porque este último hace una copia  
        }
        return act == t.end(); 
    }
    ...
    /*
    T inicial = t.begin(); 
    for (auto it = ++t.begin(); it != t.end() && inicial < *it; ++it); 
    return it == t.end(); 
    */
