...
template <typename T>
int num_nodos(const BinTree<T>& t) {
    if (t.empty()) return 0;
    else return num_nodos(t.left()) + num_nodos(t.right()) + 1; 
}

template <typename T>
int num_hojas(const BinTree<T>& t) {
    if (t.empty()) return 0;
    else if (t.left().empty() && t.right().empty()) return 1;
    else return num_hojas(t.left()) + num_hojas(t.right());
}
...
void resuelveCasos() {
    BinTree<char> t; 
    cin >> t; 
    cout << num_nodos(t) << ' ' << num_hojas(t) << ' ' << altura(t) << '\n'; 
}
