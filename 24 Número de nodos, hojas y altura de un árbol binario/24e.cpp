...
public:
    ...
    int altura() const {
        return altura(root_node); 
    }
    int hojas() const {
        return hojas(root_node);
    }
    int nodos() const{
        return nodos(root_node); 
    }
...
private:
    ...
    int altura(NodePointer t) const {
        if (t == nullptr) return 0;
        else return 1 + max(altura(t->left), altura(t->right)); 

    }
    int hojas(const NodePointer& t) const{
        if (t == nullptr) return 0;
        else if (t->right == nullptr && t->left == nullptr) return 1;
        else return hojas(t->left) + hojas(t->right); 
    }
    int nodos(const NodePointer& t) const{
        if (t == nullptr) return 0;
