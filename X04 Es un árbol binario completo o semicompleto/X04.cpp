...
template <typename T>
tuple<bool, bool, int> compsemicomp(const BinTree<T>& t) { //completo, semicompleto, altura
    if (t.empty()) return { true, true,0 }; 
    else {
        auto [cL, sL, hL] = compsemicomp(t.left()); 
        auto [cR, sR, hR] = compsemicomp(t.right()); 

        return { cL && cR && hL == hR, sL  && cR && hR == hL -1 || cL && sR && hL == hR  , hL + 1 }; 
    }
...
void resuelveCasos() { 
    BinTree<char> t; 
    cin >> t; 
    auto [comp, semi, h] = compsemicomp(t); 
    if (comp) cout << "COMPLETO";
    else if (semi) cout << "SEMICOMPLETO";
    else cout << "NADA"; 
    cout << '\n'; 
}
