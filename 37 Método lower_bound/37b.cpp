...
private:
    ...
    std::pair<bool, T> lower_bound(Node* root,T const& e) const {
        if (root == nullptr) return { false, T() };
        else if (e < root->elem) {
            auto [bL, nL] = lower_bound(root->left, e);
            if (bL) return { bL,nL };
            else return { true, root->elem };
        }
        else if (e > root->elem) return lower_bound(root->right, e);
        else return { true, root->elem }; 
    }
};
