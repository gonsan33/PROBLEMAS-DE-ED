...
public:
    ...
    SetTree(vector<T> const& elems) {
        //Hacer un �rbol equilibrado a partir de un vector 
        ...
        root_node = from_vector(copia, 0, num_elems); //elementos cuyos �ndices est�n 0<= index < num_elems
    }
    ...
    int count_interval(T const& lower, T const& upper) const {
        return this->count_interval(root_node, lower, upper);
    }
...
private:
    ...
    static Node* from_vector(vector<T> const& elems, int ini, int fin) {
        if (ini >= fin) return nullptr; //�rbol vac�o 
        else {
            int mitad = (ini + fin) / 2;
    ...
    std::pair<bool, T> lower_bound(Node* node, T const& e) const {
        if (node == nullptr) return { false, T() };
        else if (e < node->elem) {
            pair<bool, T> recursivo = lower_bound(node->left, e);
            if (recursivo.first) return recursivo;
            else return { true, node->elem };
        }
        else if (e == node->elem) return { true, e };
        else return lower_bound(node->right, e); 
    }
...
bool resuelveCasos() {
    ...
    vector<int> v(N);
    for (auto& x : v) cin >> x;
    SetTree<int> tree = SetTree<int>(v);
    cin >> M;
    for (int i = 0; i < M; ++i) {
        int ini;
        cin >> ini;
        auto rta = tree.lower_bound(ini); 
        if (rta.first) cout << rta.second << '\n';
        else cout << "NO HAY\n"; 
