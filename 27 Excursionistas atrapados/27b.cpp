...
pair<int, int> excursionistas(const BinTree<int>& t) {
    if (t.empty()) return { 0,0 }; 
        ...
        auto [eqL, exL] = excursionistas(t.left()); 
        auto [eqR, exR] = excursionistas(t.right()); 
        return { eqL + eqR + (eqL + eqR == 0 && t.root() > 0 ? 1 : 0), max(exL, exR) + t.root() }; 
    }
}
...
void resuelveCasos() {
    BinTree<int> t; 
    cin >> t; 
    pair<int, int> sol = excursionistas(t); ; 
