...
vector<int> perfil(const BinTree<int>& arbol) {
    ...
    vector<int> rta; 
    if (!arbol.empty()) {
        pending.push({ arbol, 1 });
        rta.push_back(arbol.root()); 
    }
    int lastLevel = 1; 

    while (!pending.empty()) {
        //Saco un nodo de la cola 
        auto [current, level] = pending.front(); 
        pending.pop(); 
        if (level > lastLevel) {
            rta.push_back(current.root());
            lastLevel = level; 
        }

        if (!current.left().empty()) pending.push({ current.left(), level + 1 }); 
        if (!current.right().empty()) pending.push({ current.right(), level + 1 });
    }
