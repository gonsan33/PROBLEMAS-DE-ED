...
public:
    ...
    void rotar_derecha() { //coste lineal en la longitud de la lista
        Node* toinsert = head->next; 
        Node* position = head->next->next->next; 
        int position_to_insert = 2 ; 
        dettach(toinsert); 
        ++this->num_elems; 
        while (position_to_insert < size()) {
            attach(toinsert, position); 
            dettach(position); 
            Node* aux = position; 
            position = toinsert->next->next; 
            toinsert = aux;
            position_to_insert = (position_to_insert+ 2) ;
        }
        //me queda insertar en la posición 0
        attach(toinsert, head->next); 
        --num_elems; 
