...
template <typename T>
tuple<bool, T, T> esDeBusqueda(BinTree<T>& t) {
    if (t.empty()) return { true, T(), T()};
    else {

        auto [bL, minL, maxL] = esDeBusqueda<T>(t.left());
        auto [bR, minR, maxR] = esDeBusqueda<T>(t.right());


        T min = t.left().empty() ? t.root() : minL;
        T max = t.right().empty() ? t.root() : maxR;


        return { bL && bR && (t.left().empty() || maxL < t.root()) && (t.right().empty() || minR > t.root()), min, max };
    }
}


template <typename T>
bool esDeBusquedaPubli(BinTree<T>& t) {
    auto [b, m, M] = esDeBusqueda<T>(t); 
    return b; 
}

template <typename T>
void resolver() {
    BinTree<T> t = read_tree<T>(cin);
    esDeBusquedaPubli(t) ? cout << "SI\n" : cout << "NO\n"; 
}
