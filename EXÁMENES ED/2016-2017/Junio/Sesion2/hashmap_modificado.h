...
/* Se ha modificado la clase para añadir el método erase(it), línea 341.
   El iterador al siguiente se consigue incrementando el contador recibido,
   antes de hacer nada más.
   Después se busca en la lista enlazada que contienen el nodo a borrar cuál es el
   nodo anterior, para poder enlazar los nodos de la lista enlazada saltando
   al nodo a eliminar.
 */
...
public:
    ...
    // coste lineal en el tamaño de la lista donde está el nodo a borrar,
    // si la función hash distribuye bien, en la práctica O(1)
    iterator erase(iterator const& it) {
        if (it.act == nullptr) // no está apuntando a ningún elemento
            throw std::out_of_range("fuera de la lista");
        
        // calculamos el iterador al siguiente (se devolverá)
        auto ret = it;
        ++ret;
        
        // borramos el nodo apuntado por it (está en la lista que comienza en array[it.ind])
        Link a_borrar = it.act;
        if (array[it.ind] == a_borrar) {   // es el primero de su lista
            array[it.ind] = a_borrar->sig; // saltamos el nodo a borrar
        } else { // hay que buscar el anterior...
            Link ant = array[it.ind];
            while (ant->sig != a_borrar) {
                ant = ant->sig;
            }
            // ... para enlazarlo con el siguiente, saltando así el nodo a borrar
            ant->sig = a_borrar->sig;
        }
        
        delete a_borrar; // se libera la memoria de este nodo
