...
private: 
    void swap(Nodo* anterior, Nodo* current, Nodo* siguiente) {
        if (anterior != nullptr)anterior->sig = siguiente;
        else this->prim = siguiente; 
        current->sig = siguiente->sig; 
        siguiente->sig = current; 
    }
    ...
    Nodo* desplazar(Nodo* fin) {
        ...
        while (current != fin) {
            ...
            if (siguiente != nullptr && current->elem > siguiente->elem) { //intercambio
                swap(anterior, current, siguiente); 
                anterior = siguiente; 
            }
            else { //mi nuevo current es siguiente
                anterior = current; 
                current = siguiente; 
            }
        }
