...
void ListLinkedDouble::partition(int pivot) {
    Node* first = head->next; //lleva el primer nodo cuyo valor es mayor que el pivote
    
    while (first != head && first->value <= pivot) first = first->next; 

    Node* current = first->next; 
    while (current != head && first != head) {
        Node* old_current = current; 
        current = current->next; 
        if (old_current->value <= pivot) {
            detach(old_current); 
            attach(old_current, first); 
        }
    }
}
