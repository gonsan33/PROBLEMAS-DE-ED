...
void ListLinkedList::duplicateIter() { //Coste lineal en el tamaño de la lista. 
	Node* actual = head; 
...
ListLinkedList::Node* ListLinkedList::invertir(ListLinkedList::Node*& act, ListLinkedList::Node*& ant) {
	if (act->next == nullptr) {
		...
		Node* resultado = invertir(act->next, act); 
		act->next = ant; 
		return resultado; 
	}
}
void ListLinkedList::invertirIter() {
	//head apunta al último elemento invertido
	Node* actual = head; 
	while (actual != nullptr) {
		Node* mueve = actual; 
		actual = actual->next; 
...
void ListLinkedList::invertir() {//Coste lineal en el tamaño de la lista
	if (head != nullptr) {
