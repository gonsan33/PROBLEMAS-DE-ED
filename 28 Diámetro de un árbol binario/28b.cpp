...
template <typename T> 
pair<int, int> diam(const BinTree<T>& t) { //first lleva camino más largo, second altura 
    if (t.empty()) return { 0,0 }; 
    else {
        auto [camI, hI] = diam(t.left()); 
        auto [camD, hD] = diam(t.right()); 
        return { max(camI, max(camD, hI + hD + 1)), 1 + max(hI, hD)};
    }
}
