...
private:
    ...
    const T& min(Node* root) const { //devuelve un puntero al nodo con el menor elemento del conjunto. Precondición: el árbol no es vacío
        if (root->left == nullptr) return root->elem;
        else return min(root->left); 
    }

    pair<Node*, Node*> minNode(Node* root) { //el primer elemento lleva el predecesor del nodo, el segundo el nodo. Precondición: el árbol no es vacío
